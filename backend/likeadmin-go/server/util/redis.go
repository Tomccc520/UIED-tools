...
// SetNX 函数说明：原子写入键值（仅当 key 不存在时成功），常用于幂等锁与防重放占位。
func (ru redisUtil) SetNX(key string, value interface{}, timeSec int) bool {
	ok, err := ru.redis.SetNX(
		context.Background(),
		config.Config.RedisPrefix+key,
		value,
		time.Duration(timeSec)*time.Second,
	).Result()
	if err != nil {
...
// Incr 原子递增指定 key，返回递增后的值；发生错误时返回 -1
func (ru redisUtil) Incr(key string) int64 {
	value, err := ru.redis.Incr(context.Background(), config.Config.RedisPrefix+key).Result()
	if err != nil {
...
// IncrWithExpire 原子递增并在首次写入时设置过期时间，便于实现计数窗口限流
func (ru redisUtil) IncrWithExpire(key string, timeSec int) int64 {
	value := ru.Incr(key)
	if value <= 0 {
		return value
	}
	if value == 1 && timeSec > 0 {
		ru.Expire(key, timeSec)
	}
	return value
}
