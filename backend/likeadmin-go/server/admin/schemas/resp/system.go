...
// SystemCaptchaResp 系统登录验证码返回信息
type SystemCaptchaResp struct {
	CaptchaOn     int    `json:"captchaOn"`     // 验证码开关 0/1
	CaptchaKey    string `json:"captchaKey"`    // 验证码键
	CaptchaImage  string `json:"captchaImage"`  // 验证码图片（dataURL）
	ExpireSeconds int    `json:"expireSeconds"` // 过期秒数
}
