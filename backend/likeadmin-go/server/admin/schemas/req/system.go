...
type SystemLoginReq struct {
	...
	CaptchaKey  string `json:"captchaKey" binding:"omitempty,min=12,max=80"` // 验证码键（开启验证码时必填）
	CaptchaCode string `json:"captchaCode" binding:"omitempty,min=4,max=8"`  // 验证码值（开启验证码时必填）
}
...
type SystemAuthAdminListReq struct {
	...
	IsDisable int8   `form:"isDisable,default=-1" binding:"oneof=-1 0 1"` // 是否禁用: [-1=全部, 0=启用, 1=停用]
}
...
type SystemAuthAdminUpdateReq struct {
	Nickname     string `form:"nickname" binding:"required,min=2,max=30"`      // 昵称
	Avatar       string `form:"avatar"`                                        // 头像
	Password     string `form:"password" binding:"omitempty,min=6,max=20"`     // 新密码
	CurrPassword string `form:"currPassword" binding:"omitempty,min=6,max=32"` // 当前密码
}
...
type SystemAuthRoleListReq struct {
	request.PageReq
	Keyword     string `form:"keyword"`                                                        // 角色名称/备注关键字
	MemberScope string `form:"memberScope,default=all" binding:"oneof=all hasMember noMember"` // 管理员归属范围
}
