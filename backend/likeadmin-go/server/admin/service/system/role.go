...
type ISystemAuthRoleService interface {
	...
	List(listReq req.SystemAuthRoleListReq) (res response.PageResp, e error)
...
// List 根据筛选条件获取角色分页列表
func (roleSrv systemAuthRoleService) List(listReq req.SystemAuthRoleListReq) (res response.PageResp, e error) {
	limit := listReq.PageSize
	offset := listReq.PageSize * (listReq.PageNo - 1)
	roleModel := roleSrv.db.Model(&system.SystemAuthRole{})
	if keyword := strings.TrimSpace(listReq.Keyword); keyword != "" {
		likeKeyword := "%" + keyword + "%"
		roleModel = roleModel.Where("(name LIKE ? OR remark LIKE ?)", likeKeyword, likeKeyword)
	}
	switch strings.TrimSpace(listReq.MemberScope) {
	case "hasMember":
		roleModel = roleModel.Where(
			"EXISTS (SELECT 1 FROM la_system_auth_admin a WHERE a.role = la_system_auth_role.id AND a.is_delete = ?)",
			0,
		)
	case "noMember":
		roleModel = roleModel.Where(
			"NOT EXISTS (SELECT 1 FROM la_system_auth_admin a WHERE a.role = la_system_auth_role.id AND a.is_delete = ?)",
			0,
		)
	}
	var count int64
...
func (roleSrv systemAuthRoleService) Add(addReq req.SystemAuthRoleAddReq) (e error) {
	...
	if e == nil {
		util.RedisUtil.HDel(config.AdminConfig.BackstageRolesKey, strconv.FormatUint(uint64(role.ID), 10))
	}
	return
}
...
func (roleSrv systemAuthRoleService) Edit(editReq req.SystemAuthRoleEditReq) (e error) {
	...
	err = roleSrv.db.Transaction(func(tx *gorm.DB) error {
		...
		return roleSrv.permSrv.BatchSaveByMenuIds(editReq.ID, editReq.MenuIds, tx)
	})
	e = response.CheckErr(err, "Edit Transaction err")
	if e == nil {
		util.RedisUtil.HDel(config.AdminConfig.BackstageRolesKey, strconv.FormatUint(uint64(editReq.ID), 10))
	}
	return
}
