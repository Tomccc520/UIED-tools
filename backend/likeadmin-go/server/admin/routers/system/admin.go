...
func (ah adminHandler) edit(c *gin.Context) {
	...
	passwordChanged := strings.TrimSpace(editReq.Password) != ""
	response.CheckAndRespWithData(c, gin.H{
		"passwordChanged": passwordChanged,
	}, ah.srv.Edit(c, editReq))
}
...
func (ah adminHandler) upInfo(c *gin.Context) {
	...
	passwordChanged := strings.TrimSpace(updateReq.Password) != ""
	response.CheckAndRespWithData(c, gin.H{
		"passwordChanged": passwordChanged,
	}, ah.srv.Update(c, updateReq, config.AdminConfig.GetAdminId(c)))
}
