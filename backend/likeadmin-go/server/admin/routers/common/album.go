...
func regAlbum(rg *gin.RouterGroup, group *core.GroupBase) error {
	return group.Reg(func(handle *albumHandler) {
		...
		rg.GET("/album/compressConfig", handle.albumCompressConfig)
		rg.POST("/album/compressConfigSave", middleware.RecordLog("素材压缩配置保存"), handle.albumCompressConfigSave)
		rg.POST("/album/albumRename", middleware.RecordLog("相册文件重命名"), handle.albumRename)
		rg.POST("/album/albumMetaSave", middleware.RecordLog("相册附件信息保存"), handle.albumMetaSave)
...
// albumCompressConfig 获取素材压缩配置
func (ah albumHandler) albumCompressConfig(c *gin.Context) {
	res, err := ah.srv.AlbumCompressConfig()
	response.CheckAndRespWithData(c, res, err)
}

// albumCompressConfigSave 保存素材压缩配置
func (ah albumHandler) albumCompressConfigSave(c *gin.Context) {
	var saveReq req.CommonAlbumCompressConfigSaveReq
	if response.IsFailWithResp(c, util.VerifyUtil.VerifyJSON(c, &saveReq)) {
		return
	}
	response.CheckAndResp(c, ah.srv.AlbumCompressConfigSave(saveReq))
}
...
func (ah albumHandler) albumMetaSave(c *gin.Context) {
	var saveReq req.CommonAlbumMetaSaveReq
	if response.IsFailWithResp(c, util.VerifyUtil.VerifyJSON(c, &saveReq)) {
		return
	}
	res, err := ah.srv.AlbumMetaSave(saveReq)
	response.CheckAndRespWithData(c, res, err)
}
