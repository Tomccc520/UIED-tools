...
<template>
    <div class="container">
        <a-card class="general-card" title="关注回复设置" :bordered="false">
            <a-row>
                <a-col :flex="1">
                    <a-form
                        :model="formModel"
                ...
                <a-col :flex="'86px'" class="search-actions">
                    <a-space direction="vertical" :size="18">
                        <a-button data-admin-smoke="channel-oa-reply-follow-query" type="primary" @click="search">
                            <template #icon>
                                <icon-search />
                            </template>
                            查询
                        </a-button>
                        <a-button data-admin-smoke="channel-oa-reply-follow-reset" @click="reset">
                            <template #icon>
                                <icon-refresh />
                            </template>
                            重置
            ...
            <a-alert
                class="follow-rule-alert"
                :type="followAlertType"
                :title="followAlertTitle"
                :closable="false"
                show-icon
            >
                {{ followAlertDescription }}
            </a-alert>
            ...
            <a-row class="table-operation-row">
                <a-col :span="12">
                    <a-space wrap>
                        <a-button
                            ...
                            data-admin-smoke="channel-oa-reply-follow-add"
                            type="primary"
                            @click="handleAdd"
                        >
                            <template #icon>
                ...
                <a-col :span="12" class="table-actions">
                    ...
                    <a-tooltip content="刷新">
                        <div
                            class="action-icon"
                            data-admin-smoke="channel-oa-reply-follow-refresh"
                            @click="refreshLists"
                        >
            ...
            >
                <a-alert v-if="hasEnabledConflict" type="warning" :closable="false" show-icon>
                    当前启用了 {{ enabledReplyRows.length }} 条关注回复，建议只保留一条正式生效规则。
                </a-alert>
                <a-alert v-if="selectedReplyRows.length > 0" type="info" :closable="false">
                    已选择 {{ selectedReplyRows.length }} 条关注回复，可批量删除或清空选择。
                </a-alert>
                <a-alert
            ...
            >
                ...
                <a-table-column title="回复类型" :min-width="120">
                    <template #cell="{ record }">
            ...
            <div v-if="!pager.loading && filteredReplyRows.length === 0" class="reply-empty-wrap">
                <a-empty description="暂无符合条件的关注回复规则">
                    <template #extra>
                        <a-button v-perms="['channel:oaReplyFollow:add']" type="primary" @click="handleAdd">
                            <template #icon>
                                <icon-plus />
                            </template>
                            新增第一条欢迎语
...
const followAlertType = computed(() => {
    return enabledReplyRows.value.length === 1 ? 'success' : 'warning'
})
const followAlertTitle = computed(() => {
    ...
    return `当前生效：${primaryRule.value?.name || '未命名欢迎语'}`
})
const followAlertDescription = computed(() => {
    if (!totalReplyCount.value) {
        return '请先新增一条欢迎语，用户关注公众号后会自动收到该内容。'
    }
    if (!enabledReplyRows.value.length) {
        return '用户关注后不会收到自动欢迎语，建议启用一条正式规则。'
    }
    if (hasEnabledConflict.value) {
        return '建议只保留一条正式生效规则，避免粉丝收到重复欢迎消息。'
    }
...
const refreshLists = async () => {
    ...
    setActionFeedback('info', '关注回复规则已刷新')
}

/**
 * 函数说明：清空当前批量选择状态，防止误操作。
 */
...
const handleEditSuccess = async () => {
    await getLists()
    setActionFeedback('success', '关注回复规则已保存')
}
...
const handleDelete = async (id: number) => {
    await feedback.confirm('确定要删除这条关注回复规则吗？')
...
const handleBatchDelete = async () => {
    ...
    if (!selectedReplyRows.value.length) {
        setActionFeedback('warning', '请先选择需要删除的关注回复规则')
        return
    }
    await feedback.confirm(`确定删除已选 ${selectedReplyRows.value.length} 条关注回复规则吗？`)
