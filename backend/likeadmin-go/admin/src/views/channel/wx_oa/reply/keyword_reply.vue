...
<template>
    <div class="container">
        <a-card class="general-card" title="关键词回复设置" :bordered="false">
            <a-row>
                <a-col :flex="1">
                    <a-form
                        :model="formModel"
                    ...
                    >
                        <a-row :gutter="16">
                            ...
                            <a-col :xs="24" :sm="12" :md="8">
                                <a-form-item field="matchingType" label="匹配方式">
                                    <a-select
                                        v-model="formModel.matchingType"
                ...
                <a-col :flex="'86px'" class="search-actions">
                    <a-space direction="vertical" :size="18">
                        <a-button data-admin-smoke="channel-oa-reply-keyword-query" type="primary" @click="search">
                            <template #icon>
                                <icon-search />
                            </template>
                            查询
                        </a-button>
                        <a-button data-admin-smoke="channel-oa-reply-keyword-reset" @click="reset">
                            <template #icon>
                                <icon-refresh />
                            </template>
                            重置
            ...
            <a-alert
                ...
                :type="enabledReplyRows.length > 0 ? 'success' : 'warning'"
                :title="replyAlertTitle"
                :closable="false"
                show-icon
            >
                关键词回复可同时启用多条；当多条关键词命中时，系统会优先使用排序靠前的规则。
            </a-alert>
            ...
            <a-row class="table-operation-row">
                <a-col :span="12">
                    <a-space wrap>
                        <a-button
                            ...
                            data-admin-smoke="channel-oa-reply-keyword-add"
                            type="primary"
                            @click="handleAdd"
                        >
                            <template #icon>
                ...
                <a-col :span="12" class="table-actions">
                    <a-space>
                        ...
                        <a-tag :color="enabledReplyRows.length > 0 ? 'green' : 'red'" bordered>
                            启用 {{ enabledReplyRows.length }} 条
                        </a-tag>
                    </a-space>
                    <a-tooltip content="刷新">
                        <div
                            class="action-icon"
                            data-admin-smoke="channel-oa-reply-keyword-refresh"
                            @click="refreshLists"
                        >
            ...
            <div class="reply-feedback" v-if="selectedReplyRows.length > 0 || actionFeedback.message">
                <a-alert v-if="selectedReplyRows.length > 0" type="info" :closable="false">
                    已选择 {{ selectedReplyRows.length }} 条关键词回复，可批量删除或清空选择。
                </a-alert>
                <a-alert
            ...
            >
                <a-table-column title="规则名称" data-index="name" :min-width="150" />
                <a-table-column title="关键词" data-index="keyword" :min-width="150" />
                <a-table-column title="匹配方式" :min-width="120">
                    <template #cell="{ record }">
                        {{ getMatchingType(record.matchingType) }}
                    </template>
                </a-table-column>
                ...
                <a-table-column title="回复内容" :min-width="240">
                    <template #cell="{ record }">
            ...
            <div v-if="!pager.loading && filteredReplyRows.length === 0" class="reply-empty-wrap">
                <a-empty description="暂无符合条件的关键词回复规则">
                    <template #extra>
                        <a-button v-perms="['channel:oaReplyKeyword:add']" type="primary" @click="handleAdd">
                            <template #icon>
                                <icon-plus />
                            </template>
                            新增第一条关键词规则
...
const filteredReplyRows = computed<ReplyRow[]>(() => {
    ...
    const matchingType = searchModel.value.matchingType
    ...
    return replyRows.value.filter((item) => {
        ...
        const itemMatchingType = String(Number(item.matchingType))
        const itemStatus = String(Number(item.status))
        const matchName = !name || itemName.includes(name)
        const matchKeyword = !keyword || itemKeyword.includes(keyword)
        const matchType = !matchingType || itemMatchingType === matchingType
        const matchStatus = !status || itemStatus === status
        return matchName && matchKeyword && matchType && matchStatus
    })
})
...
const replyAlertTitle = computed(() => {
    ...
    return `当前启用 ${enabledReplyRows.value.length} 条关键词回复`
})
...
const handleEditSuccess = async () => {
    await getLists()
    setActionFeedback('success', '关键词回复规则已保存')
}
...
const handleDelete = async (id: number) => {
    await feedback.confirm('确定要删除这条关键词回复规则吗？')
...
const handleBatchDelete = async () => {
    ...
    if (!selectedReplyRows.value.length) {
        setActionFeedback('warning', '请先选择需要删除的关键词回复规则')
        return
    }
    await feedback.confirm(`确定删除已选 ${selectedReplyRows.value.length} 条关键词回复规则吗？`)
