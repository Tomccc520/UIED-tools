...
const defaultChangelogTimeline = [
  {
    "id": "v3.0.2",
    "version": "3.0.2",
    "date": "2026-03-30 14:50",
    "badgeText": "会员二期",
    "badgeType": "success",
    "title": "支付状态机、订单管理页与登录弹窗体验升级",
    "features": [
      {
        "title": "会员支付链路升级（状态机）",
        "points": [
          "订单状态统一为：<strong>待支付(0) / 已支付(1) / 已关闭(2)</strong>。",
          "前台下单改为先创建待支付订单，再走支付回调，避免“下单即到账”造成状态不一致。",
          "用户中心新增待支付订单关闭能力，减少无效订单堆积。"
        ]
      },
      {
        "title": "后台新增订单管理页",
        "points": [
          "新增后台订单管理（筛选 / 导出CSV / 补单 / 关闭）。",
          "补单支持待支付、已关闭订单转已支付并补发会员权益或积分。",
          "新增菜单与权限补丁，支持一键启动脚本自动补齐。"
        ]
      },
      {
        "title": "登录弹窗视觉重构",
        "points": [
          "头部登录弹窗升级为信息侧栏 + 表单主区布局，信息密度更清晰。",
          "保留昵称密码登录与微信/QQ入口，不改变已有登录逻辑。"
        ]
      }
    ]
  },
  {
    ...
    "badgeText": "交互优化",
    "badgeType": "success",
    "title": "AI 工具箱左侧导航与全栈脚本稳定性优化",
    "features": [
      {
        "title": "左侧菜单逻辑优化",
        "points": [
          "<strong><router-link to=\"/tools/ai/toolbox\" target=\"_blank\" class=\"hover:text-blue-600 transition-colors\">AI工具箱</router-link></strong> 打开时不再整栏切换，继续保留全局分类导航，减少“页面结构突变”的违和感。",
          "AI 分类升级为分层导航：在分类内按“页面导航 / 工具分组”组织入口，避免二级菜单被平铺成一级菜单。",
          "AI 分组锚点与工具路由跳转逻辑统一，定位更稳定。"
        ]
      },
      {
        "title": "开发环境启动稳定性修复",
        "points": [
          "一键脚本强化 PID 与端口双重校验，修复“显示已运行但端口未监听”的误判问题。",
          "停止脚本新增端口兜底清理，避免 PID 过期时残留占端口进程。",
          "启动阶段增加端口就绪等待，降低抠图服务慢启动导致整体中断的概率。"
        ]
      }
    ]
  },
