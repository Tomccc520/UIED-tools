        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">研究课题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="请输入研究课题" size="large" clearable class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.type" placeholder="选择报告类型" class="w-full" size="large">
                    <el-option label="学术研究" value="academic" />
                    <el-option label="市场调研" value="market" />
                    <el-option label="实验报告" value="experiment" />
                    <el-option label="可行性研究" value="feasibility" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    ...
                    <el-option label="深度分析" value="analytical" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="500" :max="5000" :step="100" />
                  </div>
                </div>
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一份${form.length}字左右的研究报告。
研究课题：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
报告类型：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 结构严谨，包含摘要、背景、方法、结果和讨论等部分
2. 内容详实，逻辑清晰
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的研究员，擅长撰写各类学术和商业研究报告。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下研究报告进行扩写，补充数据和细节，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写研究报告，保持逻辑连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下研究报告进行摘要，提炼核心结论：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下研究报告进行改写，优化学术表达，提升专业度：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下研究报告进行润色，修正语病，使其更符合学术规范：\n\n${originalText}`
      break
    case 'fix':
