<!--
 * @file InternshipComments.vue
 * @description 实习评语生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成专业的实习评语，支持多种评价等级和风格，助您高效完成评价工作</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实习生姓名/岗位 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：张三/前端开发实习生" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：勤奋, 学习能力强, 完成项目X" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">评价等级</label>
                  <el-select v-model="form.type" placeholder="选择评价等级" class="w-full" size="large">
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="客观公正" value="objective" />
                    <el-option label="热情鼓励" value="encouraging" />
                ...
                <div>
                  ...
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="100" :max="1000" :step="50" />
                  </div>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的实习评语将在这里显示..."
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一份${form.length}字左右的实习评语。
实习生姓名/岗位：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
评价等级：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 内容具体，评价中肯
2. 结构清晰，包含工作表现、能力评估和建议
3. 语言得体，符合${getStyleLabel(form.style)}的风格
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的实习指导老师和人力资源专家，擅长撰写各类实习评语。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下实习评语进行扩写，丰富细节，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写实习评语，保持风格一致：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下实习评语进行缩写，精简语言，保留核心评价：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下实习评语进行改写，优化措辞，提升专业度，保持原意：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下实习评语进行润色，修正语病，使其更通顺得体：\n\n${originalText}`
      break
    case 'fix':
