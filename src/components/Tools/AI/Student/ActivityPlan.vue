        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">活动名称/主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：校园十佳歌手大赛/公司年会" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：青春, 活力, 预算5000元" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.type" placeholder="选择活动类型" class="w-full" size="large">
                    <el-option label="校园活动" value="campus" />
                    <el-option label="团建拓展" value="teambuilding" />
                    <el-option label="会议论坛" value="conference" />
                    <el-option label="营销促销" value="marketing" />
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="创意十足" value="creative" />
                    <el-option label="务实落地" value="practical" />
                    <el-option label="详尽周全" value="detailed" />
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我策划一份${form.length}字左右的活动方案。
活动名称/主题：${form.topic}
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个创意丰富的活动策划师，擅长策划各类精彩活动。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下活动方案进行扩写，补充活动流程和细节，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续策划活动方案，保持逻辑连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下活动方案进行摘要，提炼核心亮点：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下活动方案进行改写，优化文案表达：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下活动方案进行润色，修正语病，使其更具吸引力：\n\n${originalText}`
      break
    case 'fix':
