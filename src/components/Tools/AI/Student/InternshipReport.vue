<!--
 * @file InternshipReport.vue
 * @description 实习报告生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成各类用途的实习报告，内容规范、结构完整，满足学校和公司要求</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实习任务/项目 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：完成前端页面开发" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：Vue3, Element Plus, 性能优化" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">报告用途</label>
                  <el-select v-model="form.type" placeholder="选择报告用途" class="w-full" size="large">
                    <el-option label="学校提交" value="school" />
                    <el-option label="公司归档" value="company" />
                    <el-option label="个人记录" value="personal" />
                    <el-option label="实习证明" value="certificate" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="规范严谨" value="standard" />
                    <el-option label="详实具体" value="detailed" />
                    <el-option label="总结概括" value="summary" />
                    <el-option label="学术专业" value="academic" />
                  </el-select>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的实习报告将在这里显示..."
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一份${form.length}字左右的实习报告。
实习任务/项目：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
报告用途：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 全面记录实习过程，重点突出个人贡献
2. 结构完整，符合${getTypeLabel(form.type)}的格式要求
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个经验丰富的职场导师，擅长撰写各类规范的实习报告。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下实习报告进行扩写，补充具体细节和案例，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写实习报告，保持逻辑连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下实习报告进行摘要，提炼核心内容：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下实习报告进行改写，优化表达，提升专业度：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下实习报告进行润色，修正语病，使其更规范：\n\n${originalText}`
      break
    case 'fix':
