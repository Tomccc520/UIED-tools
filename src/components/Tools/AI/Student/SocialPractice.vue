<!--
 * @file SocialPractice.vue
 * @description 暑假社会实践心得生成工具
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实践活动主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：乡村支教/社区志愿服务" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：奉献, 团队, 艰苦奋斗" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">活动类型</label>
                  <el-select v-model="form.type" placeholder="选择活动类型" class="w-full" size="large">
                    <el-option label="支教助学" value="teaching" />
                    <el-option label="社会调研" value="survey" />
                    <el-option label="志愿服务" value="volunteer" />
                    <el-option label="勤工俭学" value="work-study" />
                    <el-option label="红色之旅" value="red-tour" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="深刻感悟" value="profound" />
                    <el-option label="生动活泼" value="lively" />
                    <el-option label="朴实真挚" value="sincere" />
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的社会实践心得将在这里显示..."
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一篇${form.length}字左右的暑假社会实践心得。
实践活动主题：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
活动类型：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 详细描述实践过程，分享感人故事或难忘瞬间
2. 总结实践收获，升华主题
3. 语言真挚，符合${getStyleLabel(form.style)}的风格
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个热心公益、乐于分享的社会实践优秀学生代表。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下社会实践心得进行扩写，补充细节，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写社会实践心得，保持情感连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下社会实践心得进行摘要，提炼核心感悟：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下社会实践心得进行改写，提升文采：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下社会实践心得进行润色，修正语病，使其更动人：\n\n${originalText}`
      break
    case 'fix':
