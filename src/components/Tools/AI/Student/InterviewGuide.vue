<!--
 * @file InterviewGuide.vue
 * @description 面试指南生成工具
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">面试岗位 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：产品经理/Java开发工程师" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：互联网大厂, 校招, 压力面" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">指南类型</label>
                  <el-select v-model="form.type" placeholder="选择指南类型" class="w-full" size="large">
                    <el-option label="常见问题与回答" value="qa" />
                    <el-option label="面试技巧" value="tips" />
                    <el-option label="注意事项" value="notice" />
                    <el-option label="着装礼仪" value="etiquette" />
                    <el-option label="全流程指南" value="full" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    ...
                    <el-option label="简明扼要" value="concise" />
                  </el-select>
                </div>
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的面试指南将在这里显示..."
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我生成一份${form.length}字左右的面试指南。
面试岗位：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
指南类型：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 针对该岗位提供具体的面试建议和问题解答
2. 内容实用，具有指导意义
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的人力资源经理和职业规划师，擅长提供专业的面试指导。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下面试指南进行扩写，补充更多细节和案例，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写面试指南，保持逻辑连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下面试指南进行摘要，提炼核心要点：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下面试指南进行改写，优化表达，提升专业度：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下面试指南进行润色，修正语病，使其更流畅：\n\n${originalText}`
      break
    case 'fix':
