<!--
 * @file PracticeReport.vue
 * @description 实践报告生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成各类实践报告，涵盖项目介绍、过程分析、成果展示，格式规范</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实践项目名称 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="请输入项目名称" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：创新, 实践, 成果" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">报告结构</label>
                  <el-select v-model="form.type" placeholder="选择报告结构" class="w-full" size="large">
                    <el-option label="标准格式" value="standard" />
                    <el-option label="问题解决型" value="problem-solving" />
                    <el-option label="经验分享型" value="experience" />
                    <el-option label="成果展示型" value="result" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="客观详实" value="objective" />
                    <el-option label="精炼简洁" value="concise" />
                    <el-option label="重点突出" value="highlighted" />
                    <el-option label="逻辑严密" value="logical" />
                  </el-select>
                </div>
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的实践报告将在这里显示..."
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一篇${form.length}字左右的实践报告。
实践项目名称：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
报告结构：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 完整呈现项目背景、实施过程、数据分析和结论
2. 结构严谨，层次分明
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个经验丰富的项目经理，擅长撰写各类专业实践报告。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下实践报告进行扩写，补充项目细节和数据，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写实践报告，保持逻辑连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下实践报告进行摘要，提炼核心成果：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下实践报告进行改写，优化表达，提升专业度：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下实践报告进行润色，修正语病，使其更规范：\n\n${originalText}`
      break
    case 'fix':
