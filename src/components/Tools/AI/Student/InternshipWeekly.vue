<!--
 * @file InternshipWeekly.vue
 * @description 实习周记生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成实习周记，记录每周工作内容、收获与感悟，助您轻松完成实习记录</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">本周工作内容 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="简要描述本周完成的工作" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：熟悉环境, 学习新工具, 团队合作" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实习阶段</label>
                  <el-select v-model="form.type" placeholder="选择实习阶段" class="w-full" size="large">
                    <el-option label="入职第一周" value="week1" />
                    <el-option label="实习中期" value="middle" />
                    <el-option label="实习结束前" value="end" />
                    <el-option label="常规周" value="normal" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="心得感悟" value="reflective" />
                    <el-option label="工作汇报" value="report" />
                    <el-option label="自我反思" value="introspection" />
                    <el-option label="总结规划" value="planning" />
                  </el-select>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的实习周记将在这里显示..."
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一篇${form.length}字左右的实习周记。
主要工作内容：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
实习阶段：${getTypeLabel(form.type)}
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个勤奋好学的实习生，擅长总结工作经验和撰写实习周记。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下实习周记进行扩写，补充工作细节和感悟，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写实习周记，保持风格一致：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下实习周记进行总结，提炼核心工作和收获：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下实习周记进行改写，优化表达，使其更条理清晰：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下实习周记进行润色，修正语病，使其更通顺自然：\n\n${originalText}`
      break
    case 'fix':
