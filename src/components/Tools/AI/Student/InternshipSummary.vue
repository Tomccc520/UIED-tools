<!--
 * @file InternshipSummary.vue
 * @description 实习总结生成工具
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实习岗位/主要成果 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：新媒体运营/文章阅读量提升20%" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：团队协作, 独立负责项目, 技能提升" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">总结侧重</label>
                  <el-select v-model="form.type" placeholder="选择总结侧重" class="w-full" size="large">
                    <el-option label="技能提升" value="skills" />
                    <el-option label="职场感悟" value="insights" />
                    <el-option label="人际交往" value="social" />
                    <el-option label="不足与展望" value="future" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="正式规范" value="formal" />
                    <el-option label="真诚感性" value="emotional" />
                    <el-option label="条理清晰" value="logical" />
                    <el-option label="简洁明了" value="simple" />
                  </el-select>
                </div>
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的实习总结将在这里显示..."
...
const generateArticle = async () => {
  if (!form.topic) {
    ElMessage.warning('请输入实习岗位或主要成果')
    return
  }
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一篇${form.length}字左右的实习总结。
实习岗位/主要成果：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
总结侧重：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 全面总结实习工作，重点突出成果和收获
2. 分析不足，提出改进计划
3. 语言规范，符合${getStyleLabel(form.style)}的风格
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个经验丰富的职场导师，擅长指导学生撰写高质量的实习总结。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下实习总结进行扩写，补充具体案例和数据，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写实习总结，保持风格一致：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下实习总结进行摘要，提炼核心价值：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下实习总结进行改写，提升职场专业度：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下实习总结进行润色，修正语病，使其更流畅优美：\n\n${originalText}`
      break
    case 'fix':
