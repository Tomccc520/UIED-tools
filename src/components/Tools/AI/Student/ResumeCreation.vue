<!--
 * @file ResumeCreation.vue
 * @description 简历内容生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成简历各模块内容，包括自我评价、工作经历、项目经验等，助您打造专业简历</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">求职意向/岗位 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：产品运营/新媒体专员" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：数据分析, 沟通协调, 英语六级" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">简历模块</label>
                  <el-select v-model="form.type" placeholder="选择简历模块" class="w-full" size="large">
                    <el-option label="自我评价" value="self-evaluation" />
                    <el-option label="工作经历" value="work-experience" />
                    <el-option label="项目经验" value="project-experience" />
                    <el-option label="技能特长" value="skills" />
                    <el-option label="校园经历" value="campus" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="专业干练" value="professional" />
                    <el-option label="数据导向" value="data-driven" />
                    <el-option label="突出亮点" value="highlight" />
                    <el-option label="诚恳踏实" value="sincere" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="100" :max="800" :step="50" />
                  </div>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的简历内容将在这里显示..."
...
import WritingGuide from '../Writing/WritingGuide.vue'
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我生成一段${form.length}字左右的简历内容。
求职意向/岗位：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
简历模块：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 针对${getTypeLabel(form.type)}模块撰写内容，突出个人优势
2. 结合关键词，展现专业能力
3. 语言${getStyleLabel(form.style)}
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的职业规划师和简历优化专家。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下简历内容进行扩写，补充更多细节，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写简历，保持风格一致：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下简历内容进行精简，突出核心优势：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下简历内容进行改写，使用更专业的职场词汇：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下简历内容进行润色，修正语病，使其更流畅有力：\n\n${originalText}`
      break
    case 'fix':
