    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成深度分析报告，运用专业模型剖析问题，提供有价值的洞察与建议</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.topic" placeholder="例如：某公司市场策略/某产品竞争力" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">关键词 (可选)</label>
                  <el-input v-model="form.keywords" placeholder="例如：优势, 劣势, 机会, 威胁" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">分析维度</label>
                  <el-select v-model="form.type" placeholder="选择分析维度" class="w-full" size="large">
                    <el-option label="SWOT分析" value="swot" />
                    <el-option label="PEST分析" value="pest" />
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="深度洞察" value="insightful" />
                    <el-option label="全面客观" value="comprehensive" />
                    <el-option label="犀利批判" value="critical" />
...
const generateArticle = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一份${form.length}字左右的分析报告。
分析对象：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
分析维度：${getTypeLabel(form.type)}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 运用${getTypeLabel(form.type)}模型进行深度剖析
2. 观点独到，论据充分
3. 语言专业，符合${getStyleLabel(form.style)}的风格
...
6. 严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的商业分析师，擅长运用各类分析模型解决复杂问题。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下分析报告进行扩写，补充论据和案例，增加字数，保持原意不变：\n\n${originalText}`
      break
    case 'continue':
      prompt = `请根据以下内容继续撰写分析报告，保持逻辑连贯：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请对以下分析报告进行摘要，提炼核心洞察：\n\n${originalText}`
      break
    case 'rewrite':
      prompt = `请对以下分析报告进行改写，提升专业度：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下分析报告进行润色，修正语病，使其更严谨：\n\n${originalText}`
      break
    case 'fix':
