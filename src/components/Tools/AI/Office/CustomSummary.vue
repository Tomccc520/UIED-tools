<!--
 * @file CustomSummary.vue
 * @description AI自定义总结工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">总结主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：新产品发布会总结、团建活动总结" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心要点</label>
                  <el-input v-model="form.points" type="textarea" :rows="4" placeholder="列出需要包含的主要内容、数据或观点..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">受众对象</label>
                  <el-input v-model="form.audience" placeholder="例如：领导、客户、全体员工" size="large" class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.topic"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-700 hover:to-fuchsia-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-purple-500/30 hover:shadow-purple-500/40 transform active:scale-[0.98]">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">总结内容</h3>
                </div>
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的总结将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
            ...
            <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
                <li>输入总结主题和核心要点，AI将生成逻辑清晰的总结文档。</li>
                <li>适用于活动总结、会议纪要、项目复盘等多种场景。</li>
            </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份关于${form.topic}的总结。
总结主题：${form.topic}
${form.points ? `核心要点：${form.points}` : ''}
${form.audience ? `受众对象：${form.audience}` : ''}

要求：
1. 结构清晰，逻辑严密，包含背景、主要内容、成果/结论、不足与改进建议等。
2. 语言客观、准确，总结到位。
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下总结进行扩写，补充更多细节和分析：\n\n${originalText}`
      break
    case 'summarize':
      prompt = `请将以下总结进行精简，提取最核心的内容：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下总结进行润色，提升专业度和可读性：\n\n${originalText}`
      break
  }
