<!--
 * @file ResignationLetter.vue
 * @description AI辞职报告生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">离职原因 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.reason" type="textarea" :rows="3" placeholder="例如：个人职业规划、家庭原因、身体原因..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">预计离职日期</label>
                  <el-input v-model="form.date" placeholder="例如：2026年3月1日" size="large" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">交接计划</label>
                  <el-input v-model="form.handover" type="textarea" :rows="2" placeholder="简述工作交接安排..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">语气风格</label>
                  <el-select v-model="form.tone" placeholder="选择语气风格" class="w-full" size="large">
                    <el-option label="感恩诚恳" value="grateful" />
                    <el-option label="客观正式" value="formal" />
                    <el-option label="委婉含蓄" value="tactful" />
                  </el-select>
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.reason"
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('grateful')" :disabled="isGenerating || !resultText"
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
            ...
            <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
                <li>输入离职原因和日期，AI将生成得体、专业的辞职报告。</li>
                <li>建议选择"感恩诚恳"风格，有助于保持良好的职业关系。</li>
            </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份正式的辞职报告。
离职原因：${form.reason}
${form.date ? `预计离职日期：${form.date}` : ''}
${form.handover ? `交接计划：${form.handover}` : ''}
语气风格：${getToneLabel(form.tone)}

要求：
1. 格式规范，包含标题、称谓、正文、落款。
2. 表达对公司的感谢，说明离职原因（得体），承诺做好交接工作。
3. 语气诚恳，不卑不亢，维护职业形象。
4. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个高情商的职场人士，擅长处理职业关系。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'grateful':
      prompt = `请将以下辞职报告修改得更加充满感激之情，回顾在公司的美好经历：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下辞职报告进行润色，使其语言更得体、专业：\n\n${originalText}`
      break
  }
