<!--
 * @file WorkReflections.vue
 * @description AI工作感想生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">感想主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：入职一周年感想、项目结项心得" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">主要经历</label>
                  <el-input v-model="form.experience" type="textarea" :rows="3" placeholder="简述相关的经历或事件..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心收获</label>
                  <el-input v-model="form.harvest" type="textarea" :rows="2" placeholder="您最大的收获或感悟是什么..."
                    class="custom-input" />
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">感想内容</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('deep')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                ...
                <template v-if="showResultEditor">
                  ...
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的感想将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
            ...
            <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
                <li>输入感想主题和相关经历，AI将生成真挚、深刻的心得体会。</li>
                <li>适用于入职感想、项目复盘、学习心得等场景。</li>
            </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份关于${form.topic}的工作感想。
感想主题：${form.topic}
${form.experience ? `主要经历：${form.experience}` : ''}
${form.harvest ? `核心收获：${form.harvest}` : ''}

要求：
1. 情感真挚，态度积极。
2. 结合具体经历，总结经验教训，提升到认知层面。
3. 语言流畅，富有感染力。
4. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个善于思考和总结的职场人士。',
      temperature: 0.8
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'deep':
      prompt = `请对以下感想进行升华，挖掘更深层次的意义和价值：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下感想进行润色，使其文字更优美、流畅：\n\n${originalText}`
      break
  }
