      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">汇报类型</label>
                  <el-select v-model="form.type" placeholder="选择汇报类型" class="w-full" size="large">
                    <el-option label="工作汇报 (通用)" value="general" />
                    <el-option label="工作日报" value="daily" />
                    <el-option label="工作周报" value="weekly" />
                    <el-option label="工作月报" value="monthly" />
                    <el-option label="项目汇报" value="project" />
                  </el-select>
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">汇报主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：12月项目进度汇报" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">完成工作</label>
                  <el-input v-model="form.done" type="textarea" :rows="3" placeholder="简述已完成的主要工作..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">进行中/计划</label>
                  <el-input v-model="form.doing" type="textarea" :rows="2" placeholder="正在进行或计划开展的工作..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">问题与困难</label>
                  <el-input v-model="form.issues" type="textarea" :rows="2" placeholder="遇到的阻碍或需要支持的事项..."
                    class="custom-input" />
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">汇报内容</h3>
                </div>
              ...
              <div class="flex-1 relative bg-white min-h-0">
                ...
                <template v-if="showResultEditor">
                  ...
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的汇报将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入汇报主题和工作内容，AI将生成结构化的工作汇报。</li>
            <li>适用于周会、月会、项目汇报等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const getTypeLabel = (type: string) => {
  ...
  return map[type] || '工作汇报'
}

const generateContent = async () => {
  if (!form.topic) {
    ElMessage.warning('请输入汇报主题')
    return
  }
...
汇报主题：${form.topic}
${form.done ? `完成工作：${form.done}` : ''}
${form.doing ? `进行中/计划：${form.doing}` : ''}
${form.issues ? `问题与困难：${form.issues}` : ''}

要求：
1. 结构清晰，符合${typeLabel}的规范。
2. 语言简练，数据详实，突出重点。
3. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的职场人士。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'summary':
      prompt = `请为以下汇报生成一份简短的摘要（200字以内）：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下汇报进行润色，使其更具条理和说服力：\n\n${originalText}`
      break
  }
