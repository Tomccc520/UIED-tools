<!--
 * @file InterviewOutline.vue
 * @description AI访谈提纲生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">访谈主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：用户需求深度访谈、专家访谈" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">访谈对象</label>
                  <el-input v-model="form.interviewee" placeholder="例如：产品重度用户、行业资深专家" size="large"
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">访谈目的</label>
                  <el-input v-model="form.purpose" type="textarea" :rows="4" placeholder="希望通过访谈了解什么信息或验证什么假设..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">预计时长 (分钟)</label>
                  <el-slider v-model="form.duration" :min="15" :max="120" :step="15" show-input />
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">提纲预览</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('deepen')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的访谈提纲将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入访谈主题、对象和目的，AI将生成结构完整的访谈提纲。</li>
            <li>提纲包含开场白、核心问题、追问方向和结语，帮助您高效进行访谈。</li>
            <li>支持调整预计时长，AI将合理规划时间分配。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我设计一份访谈提纲。
访谈主题：${form.topic}
${form.interviewee ? `访谈对象：${form.interviewee}` : ''}
${form.purpose ? `访谈目的：${form.purpose}` : ''}
预计时长：${form.duration}分钟

要求：
1. 结构完整，包含开场白（破冰）、核心问题、追问方向和结语。
2. 问题设计由浅入深，逻辑性强。
3. 包含时间分配建议。
4. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个经验丰富的用户研究员或记者，擅长设计深度访谈提纲。',
      temperature: 0.6
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'deepen':
      prompt = `请对以下访谈提纲进行优化，增加更多深度追问的问题，以便挖掘更深层的信息：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下访谈提纲进行润色，使语言更自然、亲切，利于建立良好的访谈氛围：\n\n${originalText}`
      break
  }
