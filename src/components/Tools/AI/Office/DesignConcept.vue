<!--
 * @file DesignConcept.vue
 * @description AI设计理念工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成独特、有深度的设计理念阐述，提升设计作品的价值感与专业度</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.project" placeholder="例如：云端办公Logo设计、现代简约家居" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">设计风格</label>
                  <el-input v-model="form.style" placeholder="例如：极简主义、赛博朋克、新中式" size="large" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心元素</label>
                  <el-input v-model="form.elements" type="textarea" :rows="3" placeholder="例如：云朵、连接、蓝色、高效..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">灵感来源</label>
                  <el-input v-model="form.inspiration" type="textarea" :rows="3" placeholder="简述设计的灵感来源或背景故事..."
                    class="custom-input" />
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">理念预览</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                ...
                <button @click="handleAiAssist('elevate')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的理念将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>清晰描述项目名称和所属行业，奠定设计理念的基调。</li>
            <li>列出设计的核心视觉元素或关键词，帮助AI捕捉设计细节。</li>
            <li>利用"升华价值"功能，将设计语言提升至品牌战略或文化高度。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份设计理念阐述。
项目名称：${form.project}
${form.style ? `设计风格：${form.style}` : ''}
${form.elements ? `核心元素：${form.elements}` : ''}
${form.inspiration ? `灵感来源：${form.inspiration}` : ''}
...
4. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的设计总监，擅长用文字表达设计美学和理念。',
      temperature: 0.8
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'polish':
      prompt = `请润色上述设计理念，使语言更加优美、富有艺术感和感染力，增强画面描述。

      当前设计理念：
      ${resultText.value}`
      systemPrompt = '你是一个文采斐然的设计评论家。'
      break
    case 'expand':
      prompt = `请基于上述设计理念，拓展更多的应用场景描述和细节延展，说明设计在不同媒介下的表现。

      当前设计理念：
      ${resultText.value}`
      systemPrompt = '你是一个注重落地的设计管理者。'
      break
    case 'elevate':
      prompt = `请升华上述设计理念，将其与品牌价值、社会意义或文化内涵相结合，提升设计的高度和深度。

      当前设计理念：
      ${resultText.value}`
      systemPrompt = '你是一个具有战略眼光的品牌顾问。'
      break
  }
