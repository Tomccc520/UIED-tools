<!--
 * @file NoticeWriting.vue
 * @description AI通知撰写工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        <div class="text-center mb-10 relative">
          ...
          <h2
            class="text-4xl font-bold mb-4 relative inline-block bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-blue-600">
            免费 AI通知撰写
          </h2>
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成各类办公通知、会议通知、行政通告，格式规范，用词精准</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.subject" placeholder="例如：全员会议通知、系统升级维护" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">接收对象</label>
                  <el-input v-model="form.audience" placeholder="例如：全体员工、各部门负责人" size="large" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心内容</label>
                  <el-input v-model="form.content" type="textarea" :rows="4" placeholder="请简述通知的主要事项、时间、地点等..."
                    class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.tone" placeholder="选择语气风格" class="w-full" size="large">
                    ...
                    <el-option label="平和客观" value="neutral" />
                    <el-option label="紧急重要" value="urgent" />
                  </el-select>
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.subject"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/40 transform active:scale-[0.98]">
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
            ...
            <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
                <li>输入通知主题、接收对象和核心内容，AI将生成格式规范的通知。</li>
                <li>支持多种语气风格选择，满足不同场景需求。</li>
            </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份正式的通知。
通知主题：${form.subject}
${form.audience ? `接收对象：${form.audience}` : ''}
${form.content ? `核心内容：${form.content}` : ''}
语气风格：${getToneLabel(form.tone)}

要求：
1. 格式规范，包含标题、正文、落款（XXX）和日期。
2. 语言简练，重点突出，传达准确。
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'polish':
      prompt = `请对通知进行润色，使其更加规范得体：\n\n${originalText}`
      break
    case 'simplify':
      prompt = `请精简通知内容，保留核心信息，使其更简洁：\n\n${originalText}`
      break
  }
