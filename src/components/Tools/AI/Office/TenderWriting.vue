<!--
 * @file TenderWriting.vue
 * @description AI标书撰写工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        <div class="text-center mb-10 relative">
          <div class="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            ...
            <div class="w-64 h-64 bg-gray-400 rounded-full blur-3xl -ml-20"></div>
          </div>
          <h2
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              <h3 class="text-lg font-semibold text-gray-800 mb-6 flex items-center">
                <span class="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center mr-3">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.project" placeholder="例如：智慧城市大数据平台建设" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">招标单位</label>
                  <el-input v-model="form.client" placeholder="请输入招标单位名称" size="large" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心需求/技术要求</label>
                  <el-input v-model="form.requirements" type="textarea" :rows="4" placeholder="简述招标文件的核心需求、技术指标或服务要求..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">我方优势/解决方案亮点</label>
                  <el-input v-model="form.advantages" type="textarea" :rows="4" placeholder="我们的核心优势、技术亮点或类似案例..."
                    class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.project"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-slate-600 to-gray-600 hover:from-slate-700 hover:to-gray-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-slate-500/30 hover:shadow-slate-500/40 transform active:scale-[0.98]">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">标书预览</h3>
                </div>
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的标书方案将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入项目名称和招标要求，AI将生成符合规范的标书大纲和技术方案。</li>
            <li>建议提供详细的“核心需求”和“我方优势”，以生成更具竞争力的标书。</li>
            <li>生成的标书仅供参考，请根据实际招标文件进行调整。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份投标文件的技术方案部分（大纲及核心内容）。
项目名称：${form.project}
${form.client ? `招标单位：${form.client}` : ''}
${form.requirements ? `核心需求：${form.requirements}` : ''}
${form.advantages ? `我方优势：${form.advantages}` : ''}

要求：
1. 结构严谨，符合招投标规范。通常包含项目理解、总体设计、实施方案、质量保障、售后服务等章节。
2. 针对核心需求提出切实可行的解决方案。
3. 充分展示我方优势和亮点。
4. 语言专业、规范，逻辑性强。
5. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的投标专家，擅长编写高质量的技术标书和解决方案。',
      temperature: 0.3
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand':
      prompt = `请对以下标书内容进行扩充，补充更多技术细节和实施步骤：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对以下标书内容进行润色，使其语言更加专业、规范，更有说服力：\n\n${originalText}`
      break
  }
