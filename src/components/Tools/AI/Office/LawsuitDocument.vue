<!--
 * @file LawsuitDocument.vue
 * @description AI起诉书生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">纠纷类型 <span
                      class="text-red-500">*</span></label>
                  <el-select v-model="form.type" placeholder="选择纠纷类型" class="w-full" size="large">
                    <el-option label="民间借贷" value="loan" />
                    ...
                    <el-option label="侵权赔偿" value="tort" />
                  </el-select>
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">诉讼请求</label>
                  <el-input v-model="form.claims" type="textarea" :rows="3" placeholder="例如：判令被告偿还借款10万元及利息..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">事实与理由</label>
                  <el-input v-model="form.facts" type="textarea" :rows="4" placeholder="简述事情经过、时间地点及相关证据..."
                    class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.type"
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">文书内容</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('legal')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的文书将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
            ...
            <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
                <li>输入纠纷类型、诉讼请求和事实理由，AI将生成标准的起诉书。</li>
                <li>本工具生成的文书仅供参考，正式诉讼请咨询专业律师。</li>
            </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份民事起诉状（起诉书）。
纠纷类型：${getTypeLabel(form.type)}
诉讼请求：${form.claims}
事实与理由：${form.facts}

要求：
1. 格式严格符合法院立案标准，包含原告/被告信息（留空待填）、诉讼请求、事实与理由、具状人、日期。
2. 语言严谨、专业，引用相关法律依据（如《民法典》）。
3. 逻辑清晰，证据链完整。
4. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的律师，精通中国法律和文书写作。',
      temperature: 0.5
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'legal':
      prompt = `请为上述起诉书补充具体的法律条款引用，使其更有法理依据：\n\n${originalText}`
      break
    case 'polish':
      prompt = `请对上述起诉书进行润色，使其语言更加法言法语，严谨规范：\n\n${originalText}`
      break
  }
