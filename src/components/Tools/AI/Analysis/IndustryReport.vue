<!--
 * @file IndustryReport.vue
 * @description AI行业报告生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">行业名称 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.industry" placeholder="例如：新能源汽车行业、人工智能行业" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">分析角度</label>
                  <el-input v-model="form.angle" type="textarea" :rows="3" placeholder="例如：市场规模、技术趋势、政策影响、竞争格局..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">时间范围</label>
                  <el-input v-model="form.time_frame" placeholder="例如：近三年发展情况及未来五年预测" size="large" clearable
                    class="custom-input" />
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('trend')" :disabled="isGenerating || !resultText"
                  class="px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-violet-50 hover:text-violet-600 hover:border-violet-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的行业报告将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入行业名称和分析角度，AI将为您生成深度的行业分析报告。</li>
            <li>设定时间范围，有助于生成更具时效性的分析内容。</li>
            <li>适用于行业研究、投资决策、战略规划等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份深度的行业分析报告。
行业名称：${form.industry}
${form.angle ? `分析角度：${form.angle}` : ''}
${form.time_frame ? `时间范围：${form.time_frame}` : ''}

要求：
1. 宏观与微观相结合，涵盖市场规模、产业链分析、竞争格局、技术趋势和政策环境等维度。
2. 数据详实，观点前瞻，具有参考价值。
3. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的行业研究员，擅长进行行业深度分析和趋势预测。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'trend':
      prompt = `请基于以下行业报告，重点预测未来的发展趋势和机会点：\n\n${originalText}`
      break
    case 'risk':
      prompt = `请基于以下行业报告，深入分析行业面临的风险和挑战：\n\n${originalText}`
      break
  }
