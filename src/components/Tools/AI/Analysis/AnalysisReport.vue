      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">分析对象/主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.subject" placeholder="例如：Q3季度销售数据、用户流失原因" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">分析维度</label>
                  <el-input v-model="form.dimensions" type="textarea" :rows="3" placeholder="例如：时间维度、地域维度、用户群体..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">背景数据/信息</label>
                  <el-input v-model="form.data" type="textarea" :rows="4" placeholder="提供相关的数据或背景信息，帮助AI更准确地分析..."
                    class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.subject"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-700 hover:to-teal-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/40 transform active:scale-[0.98]">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">分析内容</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('insight')" :disabled="isGenerating || !resultText"
                  class="px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-cyan-50 hover:text-cyan-600 hover:border-cyan-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center">
                ...
                <button @click="handleAiAssist('suggestion')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的分析报告将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入分析对象和维度，AI将为您生成专业的分析报告。</li>
            <li>提供详细的背景数据或信息，可以大大提高分析的准确性。</li>
            <li>适用于业务数据分析、问题原因复盘、趋势预测等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份专业的分析报告。
分析对象/主题：${form.subject}
${form.dimensions ? `分析维度：${form.dimensions}` : ''}
${form.data ? `背景数据/信息：${form.data}` : ''}

要求：
1. 分析深入，逻辑清晰，观点明确。
2. 包含现状描述、问题分析、原因探究、改进建议等部分。
3. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的数据分析师，擅长通过数据和信息发现问题并提供解决方案。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'insight':
      prompt = `请基于以下分析报告，提供更深层次的洞察和发现：\n\n${originalText}`
      break
    case 'suggestion':
      prompt = `请基于以下分析报告，提供更具体、可落地的改进建议：\n\n${originalText}`
      break
  }
