<!--
 * @file StartupIdeas.vue
 * @description 创新创业金点子工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        <div class="text-center mb-10 relative">
          <div class="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            <div class="w-64 h-64 bg-yellow-400 rounded-full blur-3xl"></div>
            <div class="w-64 h-64 bg-amber-400 rounded-full blur-3xl -ml-20"></div>
          </div>
          <h2
            class="text-4xl font-bold mb-4 relative inline-block bg-clip-text text-transparent bg-gradient-to-r from-yellow-500 to-amber-500">
            创新创业金点子
          </h2>
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">激发创业灵感，结合您的优势与资源，提供极具潜力的商业创意</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">感兴趣的领域 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.field" placeholder="例如：养老服务、宠物经济、绿色能源" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">自身优势/资源</label>
                  <el-input v-model="form.resources" type="textarea" :rows="3"
                    placeholder="例如：有技术背景、拥有社区渠道、擅长短视频营销..." class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.field"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/40 transform active:scale-[0.98]">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">创意方案</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('model')" :disabled="isGenerating || !resultText"
                  class="px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-yellow-50 hover:text-yellow-600 hover:border-yellow-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center">
                ...
                <button @click="handleAiAssist('market')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的创业金点子将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入您感兴趣的领域和自身优势，AI将为您提供几个创新创业点子。</li>
            <li>每个点子都会包含核心价值、目标用户、盈利模式和竞争优势分析。</li>
            <li>适用于创业初期寻找方向、头脑风暴、寻找商业机会等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请基于${form.field}领域，结合我的优势/资源（${form.resources || '无特殊说明'}），提供3-5个具有创新性和可行性的创业金点子。

要求：
1. 每个点子都要包含：创意名称、核心价值主张、目标用户群体、主要盈利模式、竞争优势分析。
2. 创意要新颖，同时具备落地的可能性。
3. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个富有远见的创业导师，擅长发掘商业机会和创新点子。',
      temperature: 0.9
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'model':
      prompt = `请从上述点子中选择一个最优秀的，为其设计详细的商业模式画布（包括关键合作伙伴、关键业务、价值主张、客户关系、客户细分、核心资源、渠道通路、成本结构、收入来源）：\n\n${originalText}`
      break
    case 'market':
      prompt = `请对上述点子进行市场潜力评估，分析市场规模、增长趋势以及可能面临的挑战和风险：\n\n${originalText}`
      break
  }
