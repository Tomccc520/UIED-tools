<!--
 * @file FeasibilityStudy.vue
 * @description 可行性研究报告生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.theme" placeholder="例如：新能源汽车充电站建设项目" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-input v-model="form.background" type="textarea" :rows="3"
                    placeholder="简述项目的发起缘由、当前市场环境或政策背景..." class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">主要内容/目标</label>
                  <el-input v-model="form.content" type="textarea" :rows="3"
                    placeholder="描述项目的主要建设内容、预期目标或核心功能..." class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.theme"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/40 transform active:scale-[0.98]">
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的可行性研究报告将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入项目名称、背景和主要内容，AI将为您生成完整的可行性研究报告。</li>
            <li>报告涵盖市场前景、技术可行性、财务效益和风险对策等关键章节。</li>
            <li>适用于项目立项、投融资决策、政府申报等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份关于${form.theme}的可行性研究报告。
${form.background ? `项目背景：${form.background}` : ''}
${form.content ? `主要内容/目标：${form.content}` : ''}

要求：
1. 报告结构严谨，包含项目摘要、市场分析、建设方案、投资估算、效益分析和风险评估。
2. 数据分析深入，观点客观公正。
3. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的项目咨询顾问，擅长撰写专业的可行性研究报告。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'market':
      prompt = `请基于现有报告，深入分析市场竞争格局、目标客户群体和市场增长潜力，补充更详实的市场数据（可模拟）和趋势预测：\n\n${originalText}`
      break
    case 'risk':
      prompt = `请基于现有报告，全面识别项目可能面临的政策、市场、技术和财务风险，并提出具体的应对措施：\n\n${originalText}`
      break
  }
