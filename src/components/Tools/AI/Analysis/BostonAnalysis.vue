      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        <div class="text-center mb-10 relative">
          <div class="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            <div class="w-64 h-64 bg-fuchsia-400 rounded-full blur-3xl"></div>
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              <h3 class="text-lg font-semibold text-gray-800 mb-6 flex items-center">
                <span class="w-8 h-8 rounded-lg bg-fuchsia-100 text-fuchsia-600 flex items-center justify-center mr-3">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.target" placeholder="例如：某公司的组织架构改革" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-input v-model="form.background" type="textarea" :rows="3"
                    placeholder="简述企业现状、面临的问题或变革目标..." class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.target"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-fuchsia-500 to-pink-500 hover:from-fuchsia-600 hover:to-pink-600 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-fuchsia-500/30 hover:shadow-fuchsia-500/40 transform active:scale-[0.98]">
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('hardware')" :disabled="isGenerating || !resultText"
                  class="px-3 py-1.5 text-xs font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-fuchsia-50 hover:text-fuchsia-600 hover:border-fuchsia-200 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center">
                ...
                <button @click="handleAiAssist('software')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的7S分析报告将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入分析对象和背景信息，AI将为您生成麦肯锡7S分析报告。</li>
            <li>报告涵盖战略(Strategy)、结构(Structure)、制度(Systems)、人员(Staff)、风格(Style)、技能(Skills)和共同价值观(Shared Values)。</li>
            <li>适用于企业组织诊断、变革管理、团队效能评估等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请运用麦肯锡7S模型对${form.target}进行组织分析。
${form.background ? `背景信息：${form.background}` : ''}

要求：
1. 详细分析战略(Strategy)、结构(Structure)、制度(Systems)、人员(Staff)、风格(Style)、技能(Skills)和共同价值观(Shared Values)七个维度。
2. 分析各要素之间的相互作用和匹配程度，找出存在的问题。
3. 提出改进建议，促进组织协调发展。
4. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的管理咨询顾问，擅长运用麦肯锡7S模型进行组织诊断。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'hardware':
      prompt = `请重点分析硬件三要素（战略、结构、制度），检查它们是否协调一致，是否存在相互冲突：\n\n${originalText}`
      break
    case 'software':
      prompt = `请重点分析软件四要素（人员、风格、技能、共同价值观），评估组织文化和人力资源对战略的支撑作用：\n\n${originalText}`
      break
  }
