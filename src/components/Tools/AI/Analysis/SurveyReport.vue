<!--
 * @file SurveyReport.vue
 * @description AI调研报告生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        <div class="text-center mb-10 relative">
          <div class="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            <div class="w-64 h-64 bg-emerald-400 rounded-full blur-3xl"></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">调研主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="例如：大学生消费习惯调研" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">调研对象</label>
                  <el-input v-model="form.target" placeholder="例如：在校大学生，样本量500人" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">调研方法</label>
                  <el-input v-model="form.method" placeholder="例如：问卷调查、深度访谈、实地走访" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心发现/数据</label>
                  <el-input v-model="form.findings" type="textarea" :rows="3" placeholder="简要描述调研中发现的关键数据或现象..."
                    class="custom-input" />
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.topic"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/40 transform active:scale-[0.98]">
          ...
          <div class="lg:col-span-8">
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                ...
                <button @click="handleAiAssist('suggest')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的调研报告将在这里显示..."
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mt-8">
          ...
          <ul class="space-y-2 list-disc list-inside text-gray-600 text-sm">
            <li>输入调研主题和对象，AI将为您生成规范的调研报告。</li>
            <li>提供调研方法和核心发现，有助于生成更准确的分析结论。</li>
            <li>适用于市场调研、用户访谈、社会实践报告等场景。</li>
          </ul>
        </div>
      </div>
    </div>
...
const generateContent = async () => {
  ...
  try {
    ...
    const prompt = `请帮我撰写一份详细的调研报告。
调研主题：${form.topic}
${form.target ? `调研对象：${form.target}` : ''}
${form.method ? `调研方法：${form.method}` : ''}
${form.findings ? `核心发现/数据：${form.findings}` : ''}

要求：
1. 结构规范，包含调研背景、目的、方法、结果分析和建议措施等部分。
2. 内容客观真实，逻辑性强，数据分析深入。
3. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号。`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的调研分析师，擅长整理和分析调研数据，撰写高质量的调研报告。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'summarize':
      prompt = `请对以下调研报告进行提炼，生成一份简明扼要的摘要：\n\n${originalText}`
      break
    case 'suggest':
      prompt = `请基于以下调研报告的发现，提出更具体、可操作的决策建议：\n\n${originalText}`
      break
  }
