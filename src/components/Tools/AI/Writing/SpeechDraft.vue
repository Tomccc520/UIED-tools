<!--
 * @file SpeechDraft.vue
 * @description AI演讲稿生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成精彩演讲稿，支持多种场合和风格</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">演讲主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="请输入演讲主题" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">听众对象 (可选)</label>
                  <el-input v-model="form.audience" placeholder="例如：学生、员工、客户、公众" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">演讲风格</label>
                  <el-select v-model="form.style" placeholder="选择演讲风格" class="w-full" size="large">
                    <el-option label="激情澎湃" value="passionate" />
                    <el-option label="幽默风趣" value="humorous" />
                    <el-option label="庄重正式" value="formal" />
                    <el-option label="娓娓道来" value="storytelling" />
                  </el-select>
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">演讲时长: {{ form.duration }}分钟</label>
                  <div class="px-2">
                    <el-slider v-model="form.duration" :min="1" :max="30" :step="1" />
                  </div>
                  <p class="text-xs text-gray-500 mt-1">注：一般演讲语速约为200字/分钟，AI将据此估算字数。</p>
                </div>
...
/**
 * 生成演讲稿
 * @description 根据演讲主题、对象、风格和时长生成演讲内容
 */
const generateArticle = async () => {
  if (!form.topic) {
    ElMessage.warning('请输入演讲主题')
    return
  }
  ...
  try {
    ...
    const prompt = `请帮我写一篇关于"${form.topic}"的演讲稿。
演讲主题：${form.topic}
${form.audience ? `听众对象：${form.audience}` : ''}
演讲风格：${getStyleLabel(form.style)}
预计时长：约${form.duration}分钟（约${estimatedWords}字）

要求：
1. 开场白要吸引人，能够迅速抓住听众注意力
2. 观点鲜明，论证有力，多用故事和案例
3. 结尾有感召力，能够引发共鸣或行动
...
7. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的演讲撰稿人，擅长撰写各类风格的演讲稿，能够感染听众。',
      temperature: 0.8
    }, (content) => {
