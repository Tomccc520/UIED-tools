<!--
 * @file ProjectProposal.vue
 * @description AI项目策划方案生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <h2
            class="text-4xl font-bold mb-4 relative inline-block bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-purple-600">
            免费 AI项目策划方案
          </h2>
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成专业的项目策划方案，包含背景、目标、实施计划和预算预估</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.projectName" placeholder="例如：企业数字化转型项目" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <details class="advanced-options">
                  ...
                  <div class="mt-3 space-y-5">
                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">项目背景</label>
                      <el-input v-model="form.background" type="textarea" :rows="2" placeholder="简述项目发起的背景或原因..."
                        class="custom-input" />
                    </div>

                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">项目目标</label>
                      <el-input v-model="form.goal" type="textarea" :rows="2" placeholder="希望达成的具体目标..."
                        class="custom-input" />
                    </div>

                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">预算范围</label>
                      <el-input v-model="form.budget" placeholder="例如：50万-100万" size="large" clearable
                        class="custom-input" />
                    </div>
                  </div>
                </details>
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">方案字数: {{ form.length }}字</label>
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="500" :max="3000" :step="100" />
                  </div>
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.projectName"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/40 transform active:scale-[0.98]">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">策划方案</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('expand_detail')" :disabled="isGenerating || !resultText"
                ...
                <button @click="handleAiAssist('optimize_budget')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的策划方案将在这里显示..."
                ...
                <template v-else>
                  <div class="h-full flex flex-col items-center justify-center text-gray-500 bg-gray-50/70">
                    <p class="text-sm mb-1">点击“生成方案”后将自动加载编辑器并显示结果</p>
...
const generateContent = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我撰写一份详细的项目策划方案，字数约${form.length}字。
项目名称：${form.projectName}
${form.background ? `项目背景：${form.background}` : ''}
${form.goal ? `项目目标：${form.goal}` : ''}
${form.budget ? `预算范围：${form.budget}` : ''}

要求：
1. 方案结构完整，应包含：项目背景、项目目标、市场分析/需求分析、实施方案（分阶段）、时间进度表、资源需求、预算明细估算、预期效果/收益、风险评估与应对。
2. 内容要专业、具体，具有可操作性。
3. 逻辑清晰，条理分明。
4. 语言风格商务、专业。
...
6. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个资深的项目经理和策划专家，擅长撰写各类项目策划方案。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'expand_detail':
      prompt = `请对上述方案中的"实施方案"和"时间进度"部分进行细化，增加具体的执行步骤和时间节点，保持其他部分不变：\n\n${originalText}`
      break
    case 'optimize_budget':
      prompt = `请对上述方案中的"预算明细"部分进行优化，提供更详细的费用构成和估算依据，保持其他部分不变：\n\n${originalText}`
      break
  }
