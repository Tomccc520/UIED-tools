<!--
 * @file TrainingExperience.vue
 * @description AI培训心得生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成培训心得体会，总结学习收获和感悟</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.topic" placeholder="请输入培训主题或课程名称" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">培训时间/地点 (可选)</label>
                  <el-input v-model="form.date" placeholder="例如：2023年10月，公司会议室" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">主要收获 (可选)</label>
                  <el-input v-model="form.takeaways" type="textarea" :rows="3" placeholder="请输入您的主要收获或感悟点，帮助AI更精准地写作" class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="积极向上" value="positive" />
                    <el-option label="深刻反思" value="reflective" />
                ...
                <div>
                  ...
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="300" :max="2000" :step="100" />
                  </div>
                </div>
...
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我写一篇关于"${form.topic}"的培训心得体会。
培训主题：${form.topic}
${form.date ? `培训时间/地点：${form.date}` : ''}
${form.takeaways ? `主要收获：${form.takeaways}` : ''}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 内容真实感人，结合实际工作场景
2. 结构清晰，包含培训概况、核心内容回顾、个人心得感悟、未来行动计划
...
6. 标题请使用标准 Markdown 格式（如 # 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个职场导师，能够引导用户总结学习成果，生成高质量的培训心得。',
      temperature: 0.7
    }, (content) => {
