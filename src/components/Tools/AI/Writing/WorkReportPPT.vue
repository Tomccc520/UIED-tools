<!--
 * @file WorkReportPPT.vue
 * @description AI工作汇报PPT生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成工作汇报PPT大纲和内容脚本，助力高效汇报</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.topic" placeholder="请输入汇报主题" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">汇报对象 (可选)</label>
                  <el-input v-model="form.audience" placeholder="例如：部门领导、公司高层、客户" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心内容/数据 (可选)</label>
                  <el-input v-model="form.content" type="textarea" :rows="3" placeholder="请输入汇报的核心内容、关键数据或亮点" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">汇报风格</label>
                  <el-select v-model="form.style" placeholder="选择汇报风格" class="w-full" size="large">
                    <el-option label="专业商务" value="business" />
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">PPT页数: {{ form.pages }}页</label>
                  <div class="px-2">
                    <el-slider v-model="form.pages" :min="5" :max="30" :step="1" />
                  </div>
                </div>
...
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我生成一份"${form.topic}"的工作汇报PPT大纲和内容脚本。
汇报主题：${form.topic}
${form.audience ? `汇报对象：${form.audience}` : ''}
${form.content ? `核心内容/数据：${form.content}` : ''}
PPT风格：${getStyleLabel(form.style)}
PPT页数：约${form.pages}页

要求：
1. 结构清晰，逻辑性强，包含封面、目录、背景、主要内容、数据分析、总结展望等
2. 每一页PPT都需要提供：
   - 页面标题
   - 页面核心内容（Bullet Points）
   - 演讲备注（Speaker Notes，即这一页该说什么）
   - 设计建议（可选，如建议配图类型）
...
5. 标题请使用标准 Markdown 格式（如 # 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的PPT策划专家，擅长制作高质量的工作汇报PPT大纲和脚本。',
      temperature: 0.7
    }, (content) => {
