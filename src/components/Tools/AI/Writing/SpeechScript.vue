<!--
 * @file SpeechScript.vue
 * @description AI讲话稿生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成各类场合的领导讲话稿、致辞，得体大方，富有感染力</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">讲话场合 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.occasion" placeholder="例如：公司年会、开工仪式、庆典活动" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <details class="advanced-options">
                  ...
                  <div class="mt-3 space-y-5">
                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">听众对象</label>
                      <el-input v-model="form.audience" placeholder="例如：全体员工、合作伙伴、学生" size="large" clearable
                        class="custom-input" />
                    </div>

                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">核心主题</label>
                      <el-input v-model="form.theme" type="textarea" :rows="2" placeholder="讲话的核心思想或要点..."
                        class="custom-input" />
                    </div>
                  </div>
                </details>
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">讲话风格</label>
                  <el-select v-model="form.style" placeholder="选择讲话风格" class="w-full" size="large">
                    <el-option label="庄重严肃" value="serious" />
                    <el-option label="激情澎湃" value="passionate" />
                    <el-option label="亲切感人" value="emotional" />
                    <el-option label="幽默风趣" value="humorous" />
                    <el-option label="务实简练" value="pragmatic" />
                  </el-select>
                </div>
                ...
                <div>
                  ...
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="300" :max="3000" :step="100" />
                  </div>
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.occasion"
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">讲话稿内容</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('add_quotes')" :disabled="isGenerating || !resultText"
                ...
                <button @click="handleAiAssist('polish_tone')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的讲话稿将在这里显示..."
                ...
                <template v-else>
                  <div class="h-full flex flex-col items-center justify-center text-gray-500 bg-gray-50/70">
                    <p class="text-sm mb-1">点击“生成讲话稿”后将自动加载编辑器并显示结果</p>
...
const generateContent = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我撰写一份讲话稿，字数约${form.length}字。
讲话场合：${form.occasion}
${form.audience ? `听众对象：${form.audience}` : ''}
${form.theme ? `核心主题：${form.theme}` : ''}
讲话风格：${getStyleLabel(form.style)}

要求：
1. 结构清晰，包含恰当的称呼、开场白、正文和结束语。
2. 内容要切合场合，符合听众特点。
3. 语言得体，富有感染力。
4. 正文部分要有层次感，逻辑递进。
5. 请使用Markdown格式输出。
6. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个经验丰富的撰稿人，擅长为各种场合撰写得体的讲话稿。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'add_quotes':
      prompt = `请在以下讲话稿中适当位置加入一些名人名言或金句，增强文章的文采和说服力，保持原意：\n\n${originalText}`
      break
    case 'polish_tone':
      prompt = `请润色以下讲话稿的语气，使其更加${getStyleLabel(form.style)}，更有感染力：\n\n${originalText}`
      break
  }
