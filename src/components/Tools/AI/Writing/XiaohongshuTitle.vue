<!--
 * @file XiaohongshuTitle.vue
 * @description AI小红书爆款标题生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成吸引眼球的小红书爆款标题，提升点击率</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">笔记内容/主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.content" type="textarea" :rows="3" placeholder="请输入笔记的主要内容、主题或关键词" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">目标受众 (可选)</label>
                  <el-input v-model="form.audience" placeholder="例如：大学生、职场新人、美妆爱好者" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">生成数量: {{ form.count }}个</label>
                  <div class="px-2">
                    <el-slider v-model="form.count" :min="5" :max="20" :step="1" />
                  </div>
                </div>

                <div class="pt-4">
                  <button @click="generateArticle" :disabled="isGenerating || !form.content"
...
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我为以下笔记内容生成${form.count}个吸引人的小红书爆款标题。
笔记内容/主题：${form.content}
${form.audience ? `目标受众：${form.audience}` : ''}

要求：
1. 标题要吸引眼球，具有高点击率
2. 使用Emoji表情点缀，增加视觉冲击力
...
6. 标题请使用标准 Markdown 格式（如 # 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个小红书爆款标题专家，精通各类标题党技巧，能够极大提升笔记点击率。',
      temperature: 0.9
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  if (type === 'expand') {
    prompt = `请基于以上内容，再生成10个不同风格的爆款标题：\n\n${originalText}`
  }
