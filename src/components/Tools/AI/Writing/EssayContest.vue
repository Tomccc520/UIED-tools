<!--
 * @file EssayContest.vue
 * @description AI征文生成工具
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">征文主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.theme" placeholder="请输入征文主题" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">具体要求 (可选)</label>
                  <el-input v-model="form.requirements" type="textarea" :rows="3" placeholder="例如：弘扬正能量、结合个人经历、建党百年主题等" class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="大气磅礴" value="grand" />
                    <el-option label="真挚感人" value="sincere" />
                    <el-option label="深刻哲理" value="philosophical" />
                    <el-option label="生动叙事" value="narrative" />
                  </el-select>
                </div>
                ...
                <div class="pt-4">
                  <button @click="generateArticle" :disabled="isGenerating || !form.theme"
...
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我写一篇以"${form.theme}"为主题的征文。
征文主题：${form.theme}
${form.requirements ? `征文要求：${form.requirements}` : ''}
...
7. 标题请使用标准 Markdown 格式（如 # 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个文学造诣深厚的作家，擅长撰写各类高质量的征文稿件。',
      temperature: 0.8
    }, (content) => {
