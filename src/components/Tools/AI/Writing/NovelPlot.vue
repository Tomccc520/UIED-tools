    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成小说剧情大纲、角色设定和故事梗概</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">小说类型 <span
                      class="text-red-500">*</span></label>
                  <el-select v-model="form.genre" placeholder="选择小说类型" class="w-full" size="large">
                    <el-option label="玄幻/仙侠" value="fantasy" />
                    <el-option label="都市/言情" value="romance" />
                    <el-option label="科幻/未来" value="scifi" />
                    <el-option label="悬疑/推理" value="mystery" />
                    <el-option label="历史/穿越" value="history" />
                  </el-select>
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">核心梗概/创意 (可选)</label>
                  <el-input v-model="form.idea" type="textarea" :rows="3" placeholder="请输入您的核心创意、故事背景或想要表达的主题" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">主要角色 (可选)</label>
                  <el-input v-model="form.characters" placeholder="例如：男主-李逍遥，女主-赵灵儿" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">生成内容</label>
                  <el-select v-model="form.type" placeholder="选择生成内容" class="w-full" size="large">
                    <el-option label="完整大纲" value="outline" />
                    <el-option label="角色设定" value="character" />
                    <el-option label="开篇第一章" value="chapter1" />
                    <el-option label="世界观设定" value="world" />
                  </el-select>
                </div>

                <div class="pt-4">
                  <button @click="generateArticle" :disabled="isGenerating || !form.genre"
...
/**
 * 生成小说剧情内容
 * @description 根据小说类型、创意和角色设定生成剧情相关内容
 */
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我创作关于"${getGenreLabel(form.genre)}"类型的小说内容。
小说类型：${getGenreLabel(form.genre)}
生成内容：${getTypeLabel(form.type)}
${form.idea ? `核心梗概/创意：${form.idea}` : ''}
${form.characters ? `主要角色：${form.characters}` : ''}

要求：
1. 想象力丰富，情节跌宕起伏，人物性格鲜明
2. 构建合理的世界观和冲突
3. 语言生动，具有画面感
...
5. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的小说家和编剧，擅长构建精彩的故事情节和人物角色。',
      temperature: 0.9
    }, (content) => {
...
/**
 * AI辅助编辑
 * @description 对已生成内容执行扩写、续写、缩写、改写、润色、纠错等处理
 * @param type 辅助操作类型
 */
