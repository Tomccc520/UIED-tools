<!--
 * @file EssayWriting.vue
 * @description AI作文一键写作工具
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">作文题目/话题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.title" placeholder="请输入作文题目或话题" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">年级/水平</label>
                  <el-select v-model="form.level" placeholder="选择年级/水平" class="w-full" size="large">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">作文要求 (可选)</label>
                  <el-input v-model="form.requirements" type="textarea" :rows="3" placeholder="例如：记叙文、议论文、需要包含具体事例等" class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    ...
                    <el-option label="情感真挚" value="sincere" />
                    <el-option label="深刻哲理" value="philosophical" />
                ...
                <div>
                  ...
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="300" :max="1500" :step="100" />
                  </div>
                </div>

                <div class="pt-4">
                  <button @click="generateArticle" :disabled="isGenerating || !form.title"
...
/**
 * 生成作文内容
 * @description 根据作文题目、年级和写作风格生成作文
 */
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我写一篇题为"${form.title}"的作文。
作文题目：${form.title}
年级/水平：${getLevelLabel(form.level)}
${form.requirements ? `作文要求：${form.requirements}` : ''}
...
1. 紧扣主题，立意新颖，内容充实
2. 结构完整，层次分明，过渡自然
3. 语言通顺，用词准确，符合${getLevelLabel(form.level)}水平和${getStyleLabel(form.style)}风格
...
6. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的语文老师，擅长指导和撰写各类优秀作文。',
      temperature: 0.8
    }, (content) => {
