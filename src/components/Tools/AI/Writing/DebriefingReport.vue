<!--
 * @file DebriefingReport.vue
 * @description AI述职报告生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成个人述职报告，突出工作业绩和职业亮点</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">职位/岗位名称 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.jobTitle" placeholder="请输入您的职位名称" size="large" clearable class="custom-input" />
                </div>
                ...
                <details class="advanced-options">
                  ...
                  <div class="mt-3 space-y-5">
                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">考核周期 (可选)</label>
                      <el-input v-model="form.period" placeholder="例如：2023年度、2023年第四季度、试用期" size="large" clearable
                        class="custom-input" />
                    </div>

                    <div>
                      <label class="block text-sm font-medium text-gray-700 mb-2">主要业绩/亮点 (可选)</label>
                      <el-input v-model="form.achievements" type="textarea" :rows="3" placeholder="请输入您的主要工作业绩、完成的项目或数据亮点" class="custom-input" />
                    </div>
                  </div>
                </details>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    <el-option label="客观务实" value="pragmatic" />
                    <el-option label="自信进取" value="confident" />
                    <el-option label="诚恳谦逊" value="modest" />
                  </el-select>
                </div>
                ...
                <div class="pt-4">
                  <button @click="generateArticle" :disabled="isGenerating || !form.jobTitle"
...
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我写一份${form.jobTitle}的述职报告。
职位名称：${form.jobTitle}
${form.period ? `考核周期：${form.period}` : ''}
${form.achievements ? `主要业绩/亮点：${form.achievements}` : ''}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 重点突出工作业绩和数据成果，用事实说话
2. 结构清晰，包含工作回顾、业绩亮点、存在不足、未来规划
3. 语言专业、干练，符合${getStyleLabel(form.style)}的风格
...
6. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的职场顾问，擅长撰写高质量的述职报告和工作汇报。',
      temperature: 0.7
    }, (content) => {
