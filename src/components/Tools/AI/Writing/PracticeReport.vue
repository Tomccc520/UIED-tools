        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实践主题</label>
                  <el-input v-model="form.theme" placeholder="例如：乡村振兴调研、志愿服务活动" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实践地点/单位</label>
                  <el-input v-model="form.location" placeholder="例如：xx社区、xx公司" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">实践过程</label>
                  <el-input v-model="form.process" type="textarea" :rows="4" placeholder="简述实践的主要活动和经过..."
                    class="custom-textarea" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">主要成果/感悟</label>
                  <el-input v-model="form.result" type="textarea" :rows="4" placeholder="取得了什么成果，有什么深刻体会..."
                    class="custom-textarea" />
                </div>
              ...
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button v-if="resultText" @click="copyPreviewHtml"
                    class="px-3 py-1.5 text-sm text-gray-600 hover:text-orange-600 hover:bg-gray-100 rounded-lg transition-colors flex items-center">
                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
...
/**
 * 生成实践报告
 * @description 根据用户输入的实践主题、地点、过程和成果生成实践报告
 */
const generateContent = async () => {
  if (!form.theme) {
    ElMessage.warning('请输入实践主题')
    return
  }
  ...
  try {
    ...
    const prompt = `请帮我写一篇社会实践报告。
实践主题：${form.theme}
${form.location ? `实践地点/单位：${form.location}` : ''}
${form.process ? `实践过程：${form.process}` : ''}
${form.result ? `成果/感悟：${form.result}` : ''}

要求：
1. 格式规范，包含摘要、正文（背景、过程、结果、思考）、结束语等
2. 逻辑严密，论述深入
3. 结合实际案例，体现社会价值
4. 语言学术性与生动性相结合
...
7. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的社会实践指导老师，擅长指导学生撰写实践报告。',
      temperature: 0.7
    }, (content) => {
      // 简单的打字机效果：直接追加内容
