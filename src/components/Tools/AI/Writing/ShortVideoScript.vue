<!--
 * @file ShortVideoScript.vue
 * @description AI短视频剧本生成工具
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">剧情梗概/主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.theme" type="textarea" :rows="3" placeholder="简述视频想要表达的故事或主题..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">角色设定</label>
                  <el-input v-model="form.characters" placeholder="例如：老板（严肃）、员工（机智）" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">视频时长</label>
                  <el-select v-model="form.duration" placeholder="选择预估时长" class="w-full" size="large">
                    <el-option label="15-30秒 (口播/段子)" value="short" />
                    <el-option label="30-60秒 (剧情/科普)" value="medium" />
                    <el-option label="1-3分钟 (微短剧/深度)" value="long" />
                  </el-select>
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">剧本风格</label>
                  <el-select v-model="form.style" placeholder="选择剧本风格" class="w-full" size="large">
                    <el-option label="搞笑反转" value="funny" />
                    <el-option label="情感治愈" value="healing" />
                    <el-option label="职场干货" value="career" />
                    <el-option label="悬疑推理" value="suspense" />
                    <el-option label="生活Vlog" value="vlog" />
                  </el-select>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">分镜头脚本</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('add_camera')" :disabled="isGenerating || !resultText"
                ...
                <button @click="handleAiAssist('add_music')" :disabled="isGenerating || !resultText"
...
const generateContent = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我创作一个短视频分镜头脚本。
剧情梗概/主题：${form.theme}
${form.characters ? `角色设定：${form.characters}` : ''}
预计时长：${getDurationLabel(form.duration)}
风格类型：${getStyleLabel(form.style)}

要求：
1. 采用标准的分镜头脚本格式（镜号、画面描述、台词/字幕、运镜/景别、音效/BGM、时长）。
2. 剧情要紧凑，有起承转合，最好有反转或爆点。
3. 画面描述要具体，具有画面感。
4. 台词要口语化，符合人物性格。
5. 开头前3秒要有黄金前三秒的吸引力设计。
6. 请使用Markdown表格格式展示脚本内容。
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'add_camera':
      prompt = `请对上述脚本中的"运镜"和"画面"部分进行专业细化，增加更具体的镜头语言（如推拉摇移、特写、中景等），保持其他内容不变：\n\n${originalText}`
      break
    case 'add_music':
      prompt = `请为上述脚本推荐更合适的背景音乐（BGM）和音效卡点，增强视频的节奏感和氛围，保持其他内容不变：\n\n${originalText}`
      break
  }
