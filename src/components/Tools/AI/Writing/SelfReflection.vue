<!--
 * @file SelfReflection.vue
 * @description AI检讨书生成工具
      ...
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          <div class="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none">
            <div class="w-64 h-64 bg-gray-400 rounded-full blur-3xl"></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">错误事项 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.event" type="textarea" :rows="2" placeholder="请简要描述犯错的具体事情..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">错误原因</label>
                  <el-input v-model="form.reason" type="textarea" :rows="2" placeholder="分析导致错误的原因（可选）..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">改正措施</label>
                  <el-input v-model="form.action" type="textarea" :rows="2" placeholder="计划采取的改正行动（可选）..."
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">字数要求: {{ form.length }}字</label>
                  <div class="px-2">
                    <el-slider v-model="form.length" :min="200" :max="2000" :step="100" />
                  </div>
                </div>

                <div class="pt-4">
                  <button @click="generateContent" :disabled="isGenerating || !form.event"
                    class="group w-full py-3.5 px-4 bg-gradient-to-r from-gray-600 to-slate-600 hover:from-gray-700 hover:to-slate-700 text-white font-medium rounded-xl transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-gray-500/30 hover:shadow-gray-500/40 transform active:scale-[0.98]">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              <div class="p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <div class="flex items-center space-x-3">
                  ...
                  <h3 class="font-medium text-gray-700">检讨书内容</h3>
                </div>
              ...
              <div class="px-3 py-2 border-b border-gray-100 bg-gray-50 flex flex-wrap gap-2">
                <button @click="handleAiAssist('more_sincere')" :disabled="isGenerating || !resultText"
                ...
                <button @click="handleAiAssist('more_profound')" :disabled="isGenerating || !resultText"
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI生成的检讨书将在这里显示..."
                ...
                <template v-else>
                  <div class="h-full flex flex-col items-center justify-center text-gray-500 bg-gray-50/70">
                    <p class="text-sm mb-1">点击“开始反省”后将自动加载编辑器并显示结果</p>
...
const generateContent = async () => {
  ...
  try {
    ...
    // 构建提示词
    const prompt = `请帮我写一份检讨书，字数约${form.length}字。
错误事项：${form.event}
${form.reason ? `错误原因分析：${form.reason}` : ''}
${form.action ? `改正措施计划：${form.action}` : ''}

要求：
1. 态度要极其诚恳，认识到自己的错误。
2. 对错误原因进行深刻的剖析，不找借口。
3. 提出具体、可行的改正措施和承诺。
4. 语言要庄重、严肃，符合检讨书的规范格式。
5. 包含适当的开头（尊敬的领导/老师等）和结尾（检讨人、日期等占位符）。
6. 请使用Markdown格式。
7. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`

    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个善于反思和表达的写作助手，能够帮助用户撰写态度诚恳、逻辑清晰的检讨书。',
      temperature: 0.7
    }, (content) => {
...
const handleAiAssist = async (type: string) => {
  ...
  switch (type) {
    case 'more_sincere':
      prompt = `请将以下检讨书修改得更加诚恳，语气更谦卑，表达出更强烈的悔过之意：\n\n${originalText}`
      break
    case 'more_profound':
      prompt = `请将以下检讨书修改得更加深刻，从思想根源上剖析错误，提升思想高度：\n\n${originalText}`
      break
  }
