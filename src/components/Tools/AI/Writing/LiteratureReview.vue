<!--
 * @file LiteratureReview.vue
 * @description AI文献综述生成工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成专业文献综述，支持多种学术领域</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.topic" placeholder="请输入研究主题" size="large" clearable class="custom-input" />
                </div>
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">文献范围/时间跨度 (可选)</label>
                  <el-input v-model="form.scope" placeholder="例如：近五年国内外研究、2010-2023年等" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.style" placeholder="选择写作风格" class="w-full" size="large">
                    ...
                    <el-option label="综述概括" value="review" />
                    <el-option label="批判性分析" value="critical" />
                  </el-select>
                </div>
...
/**
 * 生成文献综述
 * @description 根据研究主题、关键词与风格生成完整文献综述
 */
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请帮我写一篇关于"${form.topic}"的文献综述。
研究主题：${form.topic}
${form.keywords ? `关键词：${form.keywords}` : ''}
${form.scope ? `文献范围/时间跨度：${form.scope}` : ''}
写作风格：${getStyleLabel(form.style)}
篇幅长度：约${form.length}字

要求：
1. 全面梳理该领域的研究现状，逻辑严密，引用规范
2. 结构清晰，包含摘要、引言、主要研究流派/观点分析、研究方法总结、结论与展望
3. 语言专业学术，符合${getStyleLabel(form.style)}的风格
...
6. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个专业的学术顾问，能够生成高质量、严谨的文献综述。',
      temperature: 0.5
    }, (content) => {
