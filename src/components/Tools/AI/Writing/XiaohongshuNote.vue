    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">智能生成小红书风格笔记，包含Emoji和种草文案</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-4 space-y-6">
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">笔记主题 <span
                      class="text-red-500">*</span></label>
                  <el-input v-model="form.topic" placeholder="请输入笔记主题" size="large" clearable class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">推广产品/核心内容 (可选)</label>
                  <el-input v-model="form.product" placeholder="例如：某品牌洗面奶、旅游攻略、穿搭分享" size="large" clearable
                    class="custom-input" />
                </div>
                ...
                <div>
                  ...
                  <el-select v-model="form.tone" placeholder="选择语气风格" class="w-full" size="large">
                    <el-option label="真诚分享" value="sincere" />
                    <el-option label="种草安利" value="promotion" />
                    <el-option label="干货科普" value="educational" />
                    <el-option label="情绪吐槽" value="emotional" />
                  </el-select>
                </div>
...
const generateArticle = async () => {
  ...
  const requestId = await beginCoreToolRun({
    ...
    action: 'generate',
    routePath: '/tools/ai/xiaohongshu-note'
  })
  if (!requestId) return
  ...
  try {
    ...
    const prompt = `请帮我写一篇关于"${form.topic}"的小红书笔记。
笔记主题：${form.topic}
${form.product ? `推广产品/内容：${form.product}` : ''}
语气风格：${getToneLabel(form.tone)}
篇幅长度：约${form.length}字

要求：
1. 标题吸引人，包含表情包，具有点击欲望
2. 正文分段清晰，大量使用Emoji表情😊，增加亲和力
3. 语气自然真诚，像朋友聊天一样，符合小红书社区氛围
4. 结尾包含互动引导和相关话题标签（#）
...
7. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    const generatedResult = await generateAIWriting({
      prompt,
      systemPrompt: '你是一个小红书爆款笔记创作者，擅长撰写高互动、高转化的种草文案。',
      temperature: 0.9
    }, (content) => {
