<!--
 * @file XiaohongshuRewrite.vue
 * @description AI小红书笔记改写工具
    ...
    <div class="mx-auto">
      <div class="bg-white rounded-xl p-8 mb-4 shadow-sm">
        ...
        <div class="text-center mb-10 relative">
          ...
          <p class="text-gray-500 text-lg max-w-2xl mx-auto relative z-10">一键将普通文本改写为小红书风格笔记，增加吸引力</p>
        </div>
            ...
            <div class="bg-gray-50 rounded-xl p-6 border border-gray-100 sticky top-4">
              ...
              <div class="space-y-5">
                <div>
                  ...
                  <el-input v-model="form.originalText" type="textarea" :rows="6" placeholder="请输入需要改写的原文内容" class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">改写侧重点 (可选)</label>
                  <el-input v-model="form.focus" placeholder="例如：突出产品优势、强调使用感受、增加互动" size="large" clearable
                    class="custom-input" />
                </div>

                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">改写风格</label>
                  <el-select v-model="form.style" placeholder="选择改写风格" class="w-full" size="large">
                    <el-option label="种草安利" value="promotion" />
                    <el-option label="情感共鸣" value="emotional" />
                    <el-option label="幽默吐槽" value="humorous" />
                    <el-option label="干货分享" value="educational" />
                  </el-select>
                </div>

                <div class="pt-4">
                  <button @click="generateArticle" :disabled="isGenerating || !form.originalText"
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[900px] bg-white shadow-sm">
              ...
              <div class="flex-1 relative bg-white min-h-0">
                <template v-if="showResultEditor">
                  <v-md-editor v-model="resultText" height="100%" :mode="mode" placeholder="AI改写的内容将在这里显示..."
                ...
                <template v-else>
                  <div class="h-full flex flex-col items-center justify-center text-gray-500 bg-gray-50/70">
                    <p class="text-sm mb-1">点击“开始改写”后将自动加载编辑器并显示结果</p>
...
const generateArticle = async () => {
  ...
  try {
    ...
    const prompt = `请将以下内容改写成小红书风格的笔记。
原文内容：
${form.originalText}

${form.focus ? `改写侧重点：${form.focus}` : ''}
改写风格：${getStyleLabel(form.style)}

要求：
1. 提炼核心观点，使用吸引人的标题
2. 语言生动活泼，大量使用Emoji表情😊
3. 结构分段清晰，阅读体验好
4. 结尾包含互动引导和相关话题标签（#）
...
7. 标题请使用 Markdown 三级标题格式（### 标题），严禁在标题行使用 ** 加粗符号`
    ...
    await generateAIWriting({
      prompt,
      systemPrompt: '你是一个小红书爆款笔记改写专家，擅长将普通文本转化为高吸引力的社交媒体文案。',
      temperature: 0.8
    }, (content) => {
