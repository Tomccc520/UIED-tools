...
export default function FocusMode() {
  ...
  // 从localStorage加载数据
  useEffect(() => {
    const savedPixelData = localStorage.getItem(perlerStorageKeys.focusPixelData);
    const savedGridDimensions = localStorage.getItem(perlerStorageKeys.focusGridDimensions);
    const savedColorCounts = localStorage.getItem(perlerStorageKeys.focusColorCounts);
    const savedColorSystem = localStorage.getItem(perlerStorageKeys.focusColorSystem);
    ...
    if (savedPixelData && savedGridDimensions && savedColorCounts) {
      try {
        ...
        // 重定向到主页面
        window.location.href = buildPerlerPath();
      }
    } else {
      // 没有数据，重定向到主页面
      window.location.href = buildPerlerPath();
