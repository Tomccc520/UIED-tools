...
const canvasHandle = (type: string) => {
  ...
  switch (type) {
    ...
    case "data":
      //更新数据
      // 仪表盘取第一个数据
      const val = Number(valueData.value[0]) || 0
      const name = colunmData.value[0] || '完成率'
...
//数据
const colunmData = ref(['完成率']);
const valueData = ref(['75']);
const rowsData = ref({
  0: {
    cells: {
      0: { text: '完成率' },
