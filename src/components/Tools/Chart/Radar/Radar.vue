...
const canvasHandle = (type: string) => {
  ...
  switch (type) {
    ...
    case "data":
      //更新数据
      // 雷达图数据需要特殊处理，这里假设 valueData 是指标值，colunmData 是指标名称
      // 实际雷达图通常需要多维数据，这里做简化处理，假设只有一组数据
      // 指标最大值默认取当前数据的最大值 * 1.2
      const maxVal = Math.max(...valueData.value.map(Number)) * 1.2

      myChart.value?.setOption({
        radar: {
          indicator: colunmData.value.map(item => ({ name: item, max: maxVal }))
        },
        series: [
          {
            data: [
...
//数据
const colunmData = ref(['销售', '管理', '信息技术', '客服', '研发', '市场']);
const valueData = ref(['4200', '3000', '20000', '35000', '50000', '18000']);
const rowsData = ref({
  0: {
    cells: {
      0: { text: '销售' },
...
const initSpreadsheet = () => {
  const s = new Spreadsheet("#x-spreadsheet-demo", {
    ...
    .change((data) => {
      // save data to db
      let chartData: any = toEchartsData(data)
...
const updateDataFile = (params: { file: File }) => {
  ...
  reader.onload = async function (e) {
    ...
    // console.log(toSpreadsheetData(worksheet));
