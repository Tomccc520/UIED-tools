    ...
    <div class="mx-auto">
      ...
      <div class="bg-white rounded-xl p-5 sm:p-8 mb-4 shadow-sm">
        <div class="text-center mb-8 relative">
          <h1 class="text-3xl sm:text-4xl font-bold mb-3 relative inline-flex flex-col items-center">
            <div class="relative px-4 sm:px-12">
              <span class="text-gray-800">毒鸡汤</span>
            </div>
          </h1>
        ...
        <div class="flex justify-center gap-4">
          <button type="button" aria-label="换一句毒鸡汤" @click="getRandomQuote"
...
/**
 * 函数说明：通过同域 Go API 获取随机毒鸡汤，请求失败时继续使用页面内置内容。
 */
const getRandomQuote = async () => {
  try {
    ...
    try {
      const result = await fetchCopywritingText('poison-soup')
      currentQuote.value = result.text
      typeText(result.text)
      return
    } catch (error) {
      console.log('同域毒鸡汤接口请求失败，使用本地数据:', error)
    }
